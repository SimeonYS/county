...
class CountyPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `county`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM county WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO county (date, title, link, content)"
